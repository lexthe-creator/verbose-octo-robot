...
3. **Burn bar** — 2px track, fills based on % of waking day elapsed (6am-11pm). Left: "X% of day gone". Right: the next Today Timeline item, or "Clear for now" when nothing remains. It does not duplicate the current hero action detail.
...
6. **Training card** — gated by `settings.modules.fitness !== false`. Shows today's generated workout. When the hero next action is the workout, this card becomes supporting detail only: subtitle "Warmup · Main · Cooldown" and a smaller secondary start button at the bottom. `Start ->` calls `onStartWorkout(workout)`; green "✓ Completed" state when `fitnessState.todayComplete`. Rest days render without a start button.
